...
files = ["Run2016B","Run2016C","Run2016D"]
for f in files:
  file = TFile.Open('/shome/thaarres/EXOVVAnalysisRunII/AnalysisOutput/80X/ExoDiBosonAnalysis.JetHT_%s_trigger.root'%f, 'READ') 
  CMS_lumi.lumi_13TeV = "%s"%f
  ...
  c.Update()
  if opts.save:
    canvasname ="%striggereffMjj-ALL_noTag_%s.pdf"%(outpath,f)
  ...
  c.Update()
  if opts.save:
    canvasname ="%striggereffMjj-ALL_SingleTag_%s.pdf"%(outpath,f)
    print "saving to Canvas  %s" %canvasname
    c.Print(canvasname,"pdf")
    c.Print(canvasname.replace(".pdf",".root"),"root")
  time.sleep(opts.time) 
  ...
  c.Update()
  if opts.save:
    canvasname ="%striggereffMjj-ALL_DoubleTag_%s.pdf"%(outpath,f)
    print "saving to Canvas  %s" %canvasname
    c.Print(canvasname,"pdf")
    c.Print(canvasname.replace(".pdf",".root"),"root")
  time.sleep(opts.time) 
