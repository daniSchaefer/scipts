...
def doFit(eff,l,histtmp,end):
  ...
  start = histtmp.GetBinCenter(histtmp.FindFirstBinAbove(0.80))
...
file = TFile.Open('/shome/thaarres/EXOVVAnalysisRunII/AnalysisOutput/80X/ExoDiBosonAnalysis.JetHT_triggerZZ.root', 'READ')   
...
for h in histonames3:
  ...
  myfit = doFit(eff,l,histtmp,1200.)
  mg.Add(eff)
...
c.Update()
if opts.save:
  canvasname ="%striggereffMjj-ALL_noTag_qZ.pdf"%outpath
...
# #-------------------------vs Mjj, Single V-tag-------------------------------#
label = 'Dijet invariant mass (GeV)'
addInfo1 = "Single V-tag qZ"
addInfo2 = '85 GeV < M_{P} < 105 GeV'
...
c.Update()
if opts.save:
  canvasname ="%striggereffMjj-ALL_SingleTag_qZ.pdf"%outpath
  print "saving to Canvas  %s" %canvasname
  c.Print(canvasname,"pdf")
  c.Print(canvasname.replace(".pdf",".root"),"root")
time.sleep(opts.time) 
...
c.Update()
if opts.save:
  canvasname ="%striggereffMjj-ALL_DoubleTag_ZZ.pdf"%outpath
  print "saving to Canvas  %s" %canvasname
  c.Print(canvasname,"pdf")
  c.Print(canvasname.replace(".pdf",".root"),"root")
time.sleep(opts.time) 
